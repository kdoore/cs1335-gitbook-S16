...
Currently, the drawing application below has a default pattern that is drawn when the mouse is pressed and over the drawing-canvas area.  Extend the program below to include 3 different pattern buttons, where one pattern functions as an eraser by drawing white ellipses with no-stroke.   It will be necessary to add a state variable: ``activePattern`` that can keep track of the currently selected pattern. Three buttons can be created to allow the user to select which pattern should be active.  In the mouseClicked event, it will be necessary to check to see which button is activated if a user clicks to change the brush pattern.  
